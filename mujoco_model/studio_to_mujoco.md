...
- Mac: `/Users/<ユーザー名>/.local/share/Stud.io/CustomParts/`

> Macでは `.local` は隠しディレクトリのためFinderでは非表示。パス入力欄に直接入力する。
>
> **注意（2026-08-12訂正）**: アプリのサポートフォルダ名は`Studio.io`ではなく`Stud.io`（`i`の後にピリオドが入らない）。誤ったパス（存在しないディレクトリ）を指定すると、カスタムパーツ（モーターローター等）が無言でスキップされ、インポート後のBlenderシーンにモーターが表示されない、という気づきにくい壊れ方をする（エラーダイアログ等は出ない）。「モーターが消えた」と感じたら、まずこのパス設定を確認すること。
